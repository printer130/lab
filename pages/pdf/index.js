...
export default function Pdf () {
  ...
  useEffect(() => {
    getOne({ cuiid: 'ckz4tqxe8000009mlowmlp5sv' }).then(setOne)
  }, [])
  ...
  if (!one) return <div>Cargando...</div>

  return (
    <>
      <PDFComponent id='pdf' json={one.data[0].json} register={register} />
      <button
        className='w-28 mt-4 text-blue-700  border border-solid border-slate-600 h-11 rounded-md relative'
        onClick={handleClic}
      >
        Descargar PDF
      </button>
