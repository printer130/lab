...
export default async function (req, res) {
  ...
  if (!labId) return res.status(405).json({ data: null })
  ...
  const receiptBioUpdated = await prisma.receipts.update({
    where: {
  ...
  res.status(200).json({ data: receiptBioUpdated, ok: true, error: false })
}
